...
centroid_model = tsg_centroid_classify_full_module.get_model()#cls_full16
centroid_model.load_state_dict(torch.load("ckpt_cls/cls_full16/0102_cent_train_full16_0103.h5"))
centroid_model.cuda()
...
for batch_item in point_loader:
    ...
    gen_utils.print_3d([gen_utils.np_to_pcd(batch_item[0][0,:3,:].T)])
    for i in range(16):
        gt_cent_point = gt_centroid_coords[0,:,i:i+1].cpu().detach().numpy().T
        ...
        #moved_points_cpu+= 0.0001
        moved_points_cpu = moved_points_cpu[pred_dist_cpu<0.03]
        gen_utils.print_3d([gen_utils.np_to_pcd(gt_cent_point, color=[0,0,1]), gen_utils.np_to_pcd(sampled_points, color=[0,1,0]), gen_utils.np_to_pcd(moved_points_cpu)])
    """
