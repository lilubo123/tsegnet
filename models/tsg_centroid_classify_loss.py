...
def distance_loss(pred_distance, sample_coord, gt_cent_coord, gt_cent_exist):
    # pred_distance: B, 8, encoder_sampled_points
    ...
    # dists.shape -> B, 8, 2, encoder_sampeld_points
    dists = dists.view(dists.shape[0], 8, 2, dists.shape[2])
    dists = dists.min(dim=2)[0]

    # dists.shape -> B,8,enconder_sampled_points
...
def centroid_dist_loss(pred_distance, pred_offset, sampled_coord, gt_cent_coord, gt_cent_exist):
    ...
    #B,8,3,ecnoder_sampeld_points
    pred_offset = pred_offset.view(B, 8, 3, S_N)
    #B,8,ecnoder_sampeld_points,3
    ...
    for class_idx in range(8):
        ...
        #B, 2, encdoer_sampeld_points
        dists = square_distance(gt_cent_coord[:, class_idx*2:class_idx*2+2, :], moved_coord_in_class)
        
        #B, encoder_sampeld_points
